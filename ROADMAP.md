...
| 항목 | 상태 |
|------|------|
| 전체 진행률 | ~85% (Sprint 1-3 완료, 품질 인프라 완료) |
| 현재 Sprint | **Sprint 3 완료** — 고급 분석 + 품질 인프라 |
| Sprint 1 결과 | Core 12개 + Extension 7개 전원 완료 ✅ |
| Sprint 2 결과 | Core 4개 + Extension 4개 전원 완료 ✅ |
| Sprint 3 결과 | P5-02 영향분석 + P4-07 코드스멜 + UX 버그픽스 + 품질 인프라(CI/테스트/인터페이스) 완료 ✅ |
| 배포 | GitHub Releases v1.0.0 — win-x64 단일 실행 파일 ✅ |
| 다음 마일스톤 | Sprint 4 — 회귀 테스트 + 단축키 + 최근 폴더 |
| Sprint Velocity | Sprint 1: 19태스크 / Sprint 2: 10태스크 / Sprint 3: 6태스크+인프라 |
...
- [x] **P4-01. 노드 호버 툴팁** — GViewer `ObjectUnderMouseCursor` 이벤트 활용, 네임스페이스/필드수/메서드수/파일 경로 표시 → **Sprint 2 S2-06 완료**
- [x] **P4-02. 노드 클릭 포커스 모드** — 클릭 노드 + 1-hop 이웃 강조, 나머지 dimming, 빈 영역 클릭 시 해제 → **Sprint 2 S2-EX-01 완료**
- [x] **P4-03. 검색/필터링 기능** — Toolbar 검색 TextBox, 클래스 이름 입력 시 매칭 노드 하이라이트 → **Sprint 2 S2-05 완료**
- [x] **P4-04. 에러 상세 표시** — Error Log 패널(좌측 사이드바), 빨간 ● 인디케이터 → **Sprint 2 S2-03 완료**
- [ ] **P4-05. 메서드 호출 의존성 분석** — `VisitInvocationExpression()`, MethodCall EdgeType, 주황색 점선 → 미구현
- [x] **P4-06. 순환 의존성 감지** — DFS back-edge 탐지, 빨간색 엣지/노드 강조, StatusBar 경고 → **Sprint 2 S2-EX-02 완료**
- [x] **P4-07. 코드 스멜 지표 시각화** — Afferent/Efferent Coupling, Instability 지표, 노드 크기/색상 인코딩 → **Sprint 3 완료**
- [x] **P4-08. struct / record / enum 지원** — TypeKind 확장, 다이아몬드/헥사곤 등 노드 모양 구분 → **Sprint 2 S2-EX-03 완료**
...
- [x] **S2-02. 좌측 패널 — Namespace Filter** — 발견된 네임스페이스 체크박스 목록, 체크 변경 시 그래프 즉시 재구성
- [x] **S2-03. 좌측 패널 — Error Log** — 오너드로우 + 빨간 ● 인디케이터, All Namespaces 마스터 토글 추가
- [x] **S2-04. 우측 패널 — Class Info** — 카드 UI (키:값 행), Dependency Metrics 분리 섹션, Ca/Ce/Instability 수치 표시
- [x] **S2-05. 툴바 Search** — 클래스 이름 입력 시 매칭 노드 하이라이트 + 비매칭 노드 dimming
- [x] **S2-06. 노드 호버 툴팁** — GViewer `ObjectUnderMouseCursor` 활용, 커스텀 다크 테마 툴팁 표시

### Extension 태스크 (S2-EX)

- [x] **S2-EX-01. 노드 클릭 포커스 모드** — 클릭 노드 + 1-hop 이웃 강조, 나머지 dimming, 빈 영역 클릭 시 해제
- [x] **S2-EX-02. 순환 의존성 감지** — DFS 기반 사이클 탐지, 순환 엣지 빨간색 강조, StatusBar 경고
- [x] **S2-EX-03. struct / record / enum 지원** — TypeKind 확장, 다이아몬드/오각형 등 노드 모양 구분
- [x] **S2-EX-04. Export PNG** — 화면 직접 캡처(CopyFromScreen)로 범례 포함 PNG 저장
...
- [x] **P5-01. 네임스페이스 필터링** → Sprint 2 S2-02에서 완료
  - 사이드 패널 또는 드롭다운에 네임스페이스 목록 표시 (체크박스)
  - 선택한 네임스페이스에 해당하는 노드만 그래프에 표시
  - 필터 변경 시 실시간 그래프 업데이트

- [x] **P5-02. 변경 영향 분석** → Sprint 3에서 완료
  - 노드 선택 후 🔍 영향 분석 버튼 → 역방향 BFS N-hop 전파
  - 영향 루트(밝은 주황) + 영향 노드(어두운 주황) + 나머지 dim
  - 영향 범위를 숫자로 StatusBar에 표시, 빈 곳 클릭 시 자동 해제

- [x] **P5-03. 코드 품질 리팩토링** → Sprint 3 완료
  - IAnalyzer / IFolderScanner 인터페이스로 레이어 의존성 역전
  - xUnit 21개 단위 테스트 + GitHub Actions CI 구축
  - Directory.Packages.props NuGet 중앙 버전 관리
  - 전 public API XML 문서 주석 적용
  - ARCHITECTURE.md 코드 증거 문서화
...
- [x] **P6-01. PNG 내보내기** → Sprint 2 S2-EX-04에서 완료
- [x] **P6-00. GitHub Releases 배포** → Sprint 3 완료 — v1.0.0 win-x64 단일 실행 파일 ([다운로드](https://github.com/khwoo0527/code-archaeology-tool/releases/tag/v1.0.0))
  - Toolbar에 💾 PNG 내보내기 버튼 추가
  - CopyFromScreen으로 범례 포함 전체 캡처 후 SaveFileDialog로 저장
