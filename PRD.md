...
#### FR-04. 노드 상호작용
- **호버 툴팁**: 노드에 마우스를 올리면 아래 정보를 툴팁으로 표시
  - 네임스페이스
  - 필드 수 / 메서드 수
  - 파일 경로
- **노드 클릭 — 포커스 모드**:
  - 클릭한 노드와 직접 연결된 노드(1-hop 이웃)를 강조 표시
  - 연결되지 않은 나머지 노드는 흐리게(opacity 감소) 표시
  - 빈 영역 클릭 시 강조 해제, 전체 그래프를 정상 표시로 복원
- **검색/필터링**: 클래스 이름으로 검색 시 해당 노드를 하이라이트
...
```
┌─────────────────────────────────────────────────────┐
│  [폴더 열기]  [새로고침]  [검색: ____________]        │  ← Toolbar
├─────────────────────────────────────────────────────┤
│                                                     │
│                                                     │
│                  Graph Canvas                       │
...
- 분석 중 UI 프리징 방지: 비동기(`async/await`) 처리
...
[MsaglRenderer] ─── Microsoft.Msagl Graph 생성
        │              계층형 레이아웃 적용
        ▼
[GraphControl] ─── WinForms Panel에 렌더링
        │              호버 툴팁, 검색 하이라이트
        ▼
...
| 클래스 상속/인터페이스/필드 엣지가 시각적으로 구분된다 | 샘플 프로젝트로 확인 |
| 에러가 있는 파일이 포함된 폴더도 정상 파일은 분석된다 | 의도적으로 에러 파일 포함 후 테스트 |
| 호버 시 클래스 정보가 툴팁으로 표시된다 | 직접 확인 |
